...
class ProxmoxPerformance(BasePlugin):
    ...
    def generate_image(self, settings, device_config):
        ...
        token_id = settings.get("token_id", "").strip()
        token_secret = settings.get("token_secret", "").strip()
        ...
        if not token_id or not token_secret:
            raise RuntimeError("Proxmox API token ID and secret are required.")
        ...
        visible_guests = [
            ...
            if show_stopped or guest.get("status") != "stopped"
        ]
